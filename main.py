...
if __name__ == '__main__':
    ...
    # 高明彪改，上面是用命令行改动参数的，下面我改成直接用三个数据集中最小的数据集开跑。
    ...
    with open('data/DBLP/edges.pkl','rb') as f:
        # (PA, AP, PC, CP),
        # len(edge) == 4;
        # edge[0] == (18405, 18405)
        edges = pickle.load(f)
    with open('data/DBLP/labels.pkl','rb') as f:
        # DBLP contains three types of nodes (papers (P), authors (A), conferences (C)),
        # len(label) == 3
        # label[0] == 800, [1] == 400, [2] == 2857;
        # 每一个单位是[4052, 2]
        labels = pickle.load(f)
